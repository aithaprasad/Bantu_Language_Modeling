...
def get_char_prob(corpus,smoothing_factor):
    ...
    for key in vocabulary:
        unigram_dict[key] = smoothing_factor
        
    for i in range(len(corpus)-1):
            unigram_dict[corpus[i]] += 1

    unigram_dict = {k: v for k, v in unigram_dict.items() if v!=1}

    char_num = sum(unigram_dict.values())

    prob_dict = {}

    for key in vocabulary:
        prob_dict[key] = unigram_dict[key] /char_num
    
    print("unigrams done")

    return unigram_dict, prob_dict
...
if __name__ == "__main__":
    ...
    #cross entropy using cwe corpus: 1.9230598325441808
    #cross entropy usin Swahili probabilities: 3.3976588156115297
